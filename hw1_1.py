...
!wget "snap.stanford.edu/data/ca-GrQc.txt.gz"
...
def genErdosRenyi(N=5242, E=14484):
    ...
    ctr = 0
    while (ctr < E):
        firstRandNode = np.random.randint(0, N)
        secondRandNode = np.random.randint(0, N)
        if not Graph.IsEdge(firstRandNode, secondRandNode):
            Graph.AddEdge(firstRandNode, secondRandNode)
            ctr += 1
...
def calcClusteringCoefficientSingleNode(Node, Graph):
    ...
    C = 0.0
    neighbors = []
    nodeDeg = Graph.GetNI(Node).GetDeg()
...
def calcClusteringCoefficient(Graph):
    ...
    for i in Graph.Nodes():
        C += calcClusteringCoefficientSingleNode(i.GetId(), Graph)
